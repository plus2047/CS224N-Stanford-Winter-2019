...
def minibatch_parse(sentences, model, batch_size):
    ...
    dependencies = [None for _ in range(n_sentences)]

    partial_parses = [(i, PartialParse(s)) for i, s in enumerate(sentences)]

    a = 0
    unfinished = partial_parses[:]
    while len(unfinished):
        batch = unfinished[-batch_size:]
        del unfinished[-batch_size:]
        trans = model.predict([p[1] for p in batch])
        for act, (idx, parser) in zip(trans, batch):
            parser.parse((act,))
            if parser.finished():
                dependencies[idx] = parser.dependencies
            else:
                unfinished.append((idx, parser))
    ### END YOUR CODE
